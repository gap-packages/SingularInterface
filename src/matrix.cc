...
Obj Func_SI_bigintmat(Obj self, Obj m)
{
    ...
    if (!(IS_LIST(m) && LEN_LIST(m) > 0 && IS_LIST(ELM_LIST(m, 1)) &&
          LEN_LIST(ELM_LIST(m, 1)) > 0)) {
    ...
    for (r = 1; r <= rows; r++) {
        ...
        for (c = 1; c <= cols; c++) {
            ...
            if (!(IS_INTOBJ(t) || TNUM_OBJ(t) == T_INTPOS ||
                  TNUM_OBJ(t) == T_INTNEG)) {
                delete bim;
...
{
    if (!(IS_INTOBJ(nrrows) && IS_INTOBJ(nrcols) && INT_INTOBJ(nrrows) > 0 &&
          INT_INTOBJ(nrcols) > 0)) {
...
Obj Func_SI_SetMatElm(Obj self, Obj obj, Obj row_, Obj col_, Obj val)
{
    ...
    case SINGTYPE_BIGINTMAT_IMM: {
        ...
        number n;
        if (ISSINGOBJ(SINGTYPE_BIGINT, val) ||
            ISSINGOBJ(SINGTYPE_BIGINT_IMM, val)) {
    ...
    case SINGTYPE_MATRIX_IMM: {
        ...
        if (!(ISSINGOBJ(SINGTYPE_POLY, val) ||
              ISSINGOBJ(SINGTYPE_POLY_IMM, val)))
