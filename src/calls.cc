  ...
  public:
    TempSetCurrRing() : tmpHdl(0)
    {
        if (currRing && (!currRingHdl || IDRING(currRingHdl) != currRing)) {
            ...
            tmpHdl = enterid(" libsing fake currRingHdl ", 0, RING_CMD,
                             &IDROOT, FALSE, FALSE);
            assert(tmpHdl);
...
static Obj gapwrap(sleftv & obj, ring r)
{
    if (r == 0 && obj.RingDependend()) {
        if (currRing == 0) {
            obj.CleanUp();
            ErrorQuit("Result is ring dependent but can't figure out what "
                      "the ring should be",
                      0L, 0L);
        }
        if (currRing->ext_ref == 0) {
            ...
            if (currRing->qideal)
                ...
                NEW_SINGOBJ_ZERO_ONE(SINGTYPE_QRING_IMM, currRing, NULL,
                                     NULL);
        }
        r = currRing;
    }

    if ((obj.Typ() == RING_CMD || obj.Typ() == QRING_CMD) &&
        ((ring)obj.Data())->ext_ref != 0) {
...
Obj Func_SI_CallFunc2(Obj self, Obj ringOrZero, Obj op, Obj a, Obj b)
{
    ...
    sleftv  result;
    BOOLEAN ret =
        iiExprArith2(&result, singa.ptr(), INT_INTOBJ(op), singb.ptr());
    EndPrintCapture();
    if (ret) {
        result.CleanUp(r);
        return Fail;
...
Obj Func_SI_CallFunc3(Obj self, Obj ringOrZero, Obj op, Obj a, Obj b, Obj c)
{
    ...
    StartPrintCapture();
    sleftv  result;
    BOOLEAN ret = iiExprArith3(&result, INT_INTOBJ(op), singa.ptr(),
                               singb.ptr(), singc.ptr());
    EndPrintCapture();
    if (ret) {
        result.CleanUp(r);
...
Obj Func_SI_CallFuncM(Obj self, Obj ringOrZero, Obj op, Obj arg)
{
    ...
    sleftv  result;
    BOOLEAN ret =
        iiExprArithM(&result, nrargs ? &wrap.s_arg : NULL, INT_INTOBJ(op));
    EndPrintCapture();
