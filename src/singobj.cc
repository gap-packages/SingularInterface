...
void * FOLLOW_SUBOBJ(
    Obj proxy, int pos, void * current, int & currgtype, const char *(&error))
{
...
void SingObj::init(Obj input, ring & r)
{
    ...
    if (IS_INTOBJ(input) || TNUM_OBJ(input) == T_INTPOS ||
        TNUM_OBJ(input) == T_INTNEG) {
        ...
        if (IS_INTOBJ(ELM_PLIST(input, 2))) {
            ...
            obj.Init();
            error = "proxy objects to Singular interpreter variables are not "
                    "yet implemented";
        }
        else {
            obj.Init();
